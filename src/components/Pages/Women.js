...
export const Women = ({ womensProducts }) => {
    const [filteredProducts, setFilteredProducts] = useState(womensProducts);
        ...
        const updateProductsWithClothingFilters = () => {
            if (clothingFilter.length > 0) {
                ...
                for (let i = 0; i < clothingFilter.length; i++) {
                    ...
                    // Iterate through womensProducts array
                    for (let j = 0; j < womensProducts.length; j++) {
                        //womensProducts[j] EQUALS a product

                        // Iterate through each category in womensProducts[j]
                        for (let k = 0; k < womensProducts[j].categories.length; k++)
                            //womensProducts[j].categories[k].slug EQUALS a category name

                            // Compares the current clothing filter and the product's category name
                            if (clothingFilter[i] === womensProducts[j].categories[k].slug) {

                                // Add current product, womensProducts[j] to filteredProducts array
                                setFilteredProducts(prevState => [...prevState, womensProducts[j]]);
                            }
                    }
                }
            } else {
    ...
    }, [clothingFilter, priceFilter, womensProducts]);
        ...
        <div style={styles.container}>
            <div>
                <Breadcrumbs style={styles.breadcrumbs} aria-label="breadcrum">
                    ...
                    <Typography color="textPrimary">Women</Typography>
                </Breadcrumbs>
            </div>
            <div className="content-container">
                ...
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <h2 style={{margin: '17px 0 0 40px'}}>Showing {filteredProducts.length} women's products</h2>
