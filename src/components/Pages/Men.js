...
export const Men = ({ mensProducts }) => {
        ...
        const updateProductsWithClothingFilters = () => {
            if (clothingFilter.length > 0) {
                ...
                for (let i = 0; i < clothingFilter.length; i++) {
                    ...
                    // Iterate through mensProducts array
                    for (let j = 0; j < mensProducts.length; j++) {
                        //mensProducts[j] EQUALS a product

                        // Iterate through each category in mensProducts[j]
                        for (let k = 0; k < mensProducts[j].categories.length; k++)
                            //mensProducts[j].categories[k].slug EQUALS a category name

                            // Compares the current clothing filter and the product's category name
                            if (clothingFilter[i] === mensProducts[j].categories[k].slug) {

                                // Add current product, mensProducts[j] to filteredProducts array
                                setFilteredProducts(prevState => [...prevState, mensProducts[j]]);
                            }
                    }
                }
            } else {
    ...
    }, [clothingFilter, priceFilter, mensProducts]);
        ...
        <div style={styles.container}>
            <div>
                <Breadcrumbs style={styles.breadcrumbs} aria-label="breadcrum">
                    ...
                    <Typography color="textPrimary">Men</Typography>
                </Breadcrumbs>
            </div>
            <div className="content-container">
                ...
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <h2 style={{ margin: '17px 0 0 40px' }}>Showing {filteredProducts.length} men's products</h2>
